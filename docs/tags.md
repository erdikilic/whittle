...
updated to match. If trimming removed every modified base, all three are dropped.
...
With `--update-moves`, a crop slices `mv` and advances `ts`/`ns`, while a split
emits dorado-style subreads (`pi` parent id, `sp` parent-signal offset, `ns`
subread span, `ts` 0, `rn` -1) so the renamed segment stays locatable in POD5 for
tools like Remora.
